...
import com.sac.logics.TableManager.Rinfo;
...
class TableManager {
	...
	HashMap<Date, HashMap<String, ArrayList>> resWithDate;// Used to check availiability.
															...
															// <11:30 ->Table1>
	int Table; // tables;
	ArrayList<Request> waitingList;// Can be designed as a command pattern.When a cancelation request come, check
									// if there is available spot for customer.
	...
	public void reservResult(Request r) {
		if (processRequest(r)) {
			...
			System.out.println("You are on the waiting list or please change your reservation information.");
		}
	}
	...
	private boolean isAvailable(Date date, HashMap<String, Integer> request, TimeSpan ts){
		...
		// end = res.get(ts.end);
		 Iterator i = ((List) request).iterator();
		 while (i.hasNext()){
